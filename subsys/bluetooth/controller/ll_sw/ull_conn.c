...
/* BLEX: Parameters are newly defined for BLEX */
uint64_t start_cycle[CONFIG_BT_MAX_CONN];	  //BLEX: Each connection event's starting clock 
uint64_t stop_cycle[CONFIG_BT_MAX_CONN];	  //BLEX: Each connection event's stoping clock
uint32_t BLEX_start;			 	  //BLEX: The parameter determines whether the BLEX works well
uint32_t delta_ms[CONFIG_BT_MAX_CONN];		  //BLEX: The time difference of start_cycle and stop_cycle in msec
uint64_t delta[CONFIG_BT_MAX_CONN];	 
#define M 100				  	  //BLEX: The parameter determines how often BLEX works
uint32_t M_num = M;
uint32_t avg_conn_evt_length[CONFIG_BT_MAX_CONN]; //BLEX: Average connection event length of each connection
uint32_t handle_cnt[CONFIG_BT_MAX_CONN];	  //BLEX: The numbers of connection events' of each connection
int old_handle;					  //BLEX: The connection handle that ends right before
int first_handle;				  //BLEX: The connection handle which is connected firstly
bool update_signal = false;			  //BLEX: The flag means update is needed
bool update_is_possible = false;		  //BLEX: The parameter indicates that connection's joinging is done and the connection update is possible
#define MAX_TABLE_SIZE (CONFIG_BT_MAX_CONN*256)   //BLEX: Maximum schedule table size
uint32_t conn_int_table[MAX_TABLE_SIZE];	  //BLEX: Connection intervals of each connection
uint32_t array0[MAX_TABLE_SIZE];		  //BLEX: The array saves the connection handles of schedule table
uint32_t array1[MAX_TABLE_SIZE];		  //BLEX: The array saves the anchor points of schedule table
uint32_t array4[CONFIG_BT_MAX_CONN];	          //BLEX: We check whether a connection is blocked a lot
uint32_t temp_array1[MAX_TABLE_SIZE];		  //BLEX: The parameter is used to find maximum idle cell
uint32_t anchor_points[CONFIG_BT_MAX_CONN];	  //BLEX: This array saves the anchor points of each connection
bool scheduled_handles[CONFIG_BT_MAX_CONN];	  //BLEX: If the connection is scheduled, scheduled_handles[handle] is true
bool unscheduled_handles[CONFIG_BT_MAX_CONN];	  //BLEX: It is used for the unschedulable connection
int scheduled_handle_cnt;			  //BLEX: The number of scheduled connections
bool need_update_dummy = false;			  //BLEX: These parameters are used to protect the scheduling operation
bool need_conn_update = false;
bool need_calculation = false;
int need_schedule_cnt = 2;			  //BLEX: These parameters are used to protect the update procedure	
int need_schedule_update = 2;
int ewma = 10;					  //BLEX: The EWMA ratio

int max_conn_int;				  //BLEX; The maximum connection interval among all the connections and the connection handle of it
int max_conn_index;
#define need_cell_size 9
int max_empty_cell[need_cell_size];		  //BLEX: The location of maximum idle cell and the size of it 
uint32_t temp_max_empty_cell_size[need_cell_size];
int gap_1 = 1;					  //BLEX: The criteria that the first update occurs
int gap = 1;					  //BLEX: The criteria that the second update occurs	
int BLEX_cnt2 = 1;				  //BLEX: The parameter is used to calculate the connection event length
int old_BLEX_cnt2 = 0;
int BLEX_cnt4;					  //BLEX: The parameter is used to mark the time when the BLEX operates strangely 
int table_size = 0;				  //BLEX: The size of schedule table
int old_table_size =0;
int repeated_size;				  //BLEX: The repeated unit size
int old_repeated_size = 0;
#endif
...
#if defined(CONFIG_BLEX_UPDATE)
/* BLEX: We define new ll_conn_update function to update the win_offset */
uint8_t ll_conn_update_BLEX(uint16_t handle, uint8_t cmd, uint8_t status, uint16_t offset)
{
	struct ll_conn *conn;
...
int ull_conn_llcp(struct ll_conn *conn, uint32_t ticks_at_expire, uint16_t lazy)
{
...
#if (defined CONFIG_CSR) || (defined CONFIG_BCM)
	if(scheduled_handle_cnt ==0 )scheduled_handle_cnt = 1;
	else if(scheduled_handle_cnt ==1 && conn_handle == 1)scheduled_handle_cnt =2;
	...
	if(old_handle >=0 && old_handle < CONFIG_BT_MAX_CONN ){
		/* BLEX: If the connection event length is valid, update the connection event length table and average them */
...
{
	uint32_t gap_start = (uint32_t)k_cyc_to_ns_floor64(k_cycle_get_32());
...
#if defined(CONFIG_BLEX)
	/* BLEX: We calculate the connection event length. 
		If the connection event is terminated by CRC errer(i.e., link loss), 
		we does not use this connection event length to average connecion event length. */
	int conn_handle = lll->handle;
	...
	if(1){
		/* BLEX: We check which connection was disconnected */
		int disconnected_handle;
		bool disconnected = false;
		int disconnected_interval;
		...
		/* BLEX: We update the schedule if a connection was disconnected */
		if(disconnected && scheduled_handle_cnt != 0){	
			...
			new_table_size = temp_table_size;
			...
			/* BLEX: We handle the case, which the disconnection connection is standard connection, in the code below. */
			if(disconnected_handle == first_handle){
				...
				int temp_j = new_first_handle_index;
				for(int i = 0; i <new_repeated_size; i ++){
					...
					for(int j = 1; j < table_size/new_repeated_size; j ++){
						if(temp_temp_temp_array[(temp_j+j)%table_size] == new_first_handle){
							j= table_size/new_repeated_size;
						}
						else 
			...
			/* BLEX: We delete the disconnected connection in the schedule table.*/
			...
			for (int i = 0; i <old_old_repeated_size ; i ++){
				...
				for(int j =1 ; j<table_size/old_old_repeated_size;j ++){
					if(array0[i*table_size/old_old_repeated_size+j] == first_handle) 
						j = table_size/old_old_repeated_size;
		...
		/*BLEX: We add new connection to the schedule table.
			If the connection's interval is not schedulable, we does not add it the table. */
		static uint32_t temp_anchor_points[CONFIG_BT_MAX_CONN];
		bool start = false;		
		if(!scheduled_handles[conn_handle] && handle_cnt[conn_handle] > 0 && 2048%(conn->lll.interval) == 0){
			...
			if(scheduled_handle_cnt == 1){
				...
				/* BLEX: If there are ongoing connections, we increase the table size and re-fill the schedule table with ongoing/new connections.*/
				int temp_array[table_size], temp_temp_array[table_size];
				
				/* BLEX: We initialize the schedule table*/
				for( int i = 0; i < table_size; i ++){
				...
				/*BLEX: We add ongoint connections to the table and mark the anchor points*/
				...
				/* BLEX: Then, we fill the new connection to the table*/
							...
							if( (array1[j] > anchor_point&& j < table_size) || j ==table_size){
								if(array0[j-1] == -1){
									...
									if((array0[j-1] != conn_handle&& j>0)|| j == 0) {
										while(array0[j-1] == -1 && array1[j-1] == array1[j] && j-1>=0)
											j --;
			...
			/*BLEX: If the new connection's interval is not schedulable, we does not add it to the table*/
			unscheduled_handles[conn_handle] = true;
			need_update_dummy = false;
			start = true;
		}
		/* BLEX: We calculate the winoffset for future connections */
		if( start || (!need_update_dummy && handle_cnt[max_conn_index] > 1 && handle_cnt[max_conn_index] % M_num == 0  && conn_handle == max_conn_index && anchor_points[conn_handle] != 0 && scheduled_handles[conn_handle])){
			...
			/*BLEX: If the new connection is added and it needs be scheduled, we fill the schedule table with the new schedule */
			bool flag = false;
			for(int i = 0; i < CONFIG_BT_MAX_CONN; i ++)
			...
			if(!flag){
				...
				/*BLEX: The anchor points can be outside the range of the repeated unit size.
					We re-arrange these anchor points within an repeated unit size. */
				int standard_anchor_point =0;
				bool flag = true;	
				...
				if( flag == false) standard_anchor_point --;
				...
				/*BLEX: We fill the idle cell's starting points */
				for(int i =1; i<table_size; i ++)
					if(array0[i] == -1)
						if(array0[i-1] != -1) array1[i] = array1[i-1] + avg_conn_evt_length[array0[i-1]];
						else array1[i] = array1[i-1];
				...
				/*BLEX: We pre-calculate the max empty cell's starting points according to the future new connection's interval*/
				...
				/* BLEX: We save the max empty cell's position in the table*/
...
#if defined CONFIG_BLEX_UPDATE
		/* BLEX: We update the schedule if it is efficient */
			...
			/* BLEX: We update the schedule long after all the connections are scheduled */
			if(conn_int_table[scheduled_handle_cnt-1] >0){
				if( handle_cnt[scheduled_handle_cnt-1] >40000/conn_int_table[scheduled_handle_cnt-1]/1.25) {
					/* BLEX: We order the connection considering the first anchor points of each connection, within the repeated unit.*/
					need_conn_update = false;
					uint32_t schedule[CONFIG_BT_MAX_CONN];
					uint32_t scheduled_dist[CONFIG_BT_MAX_CONN];
					bool tempbool = false;
					...
					for(int i = 0; i <CONFIG_BT_MAX_CONN; i ++){
						for(int j = i+1; j < CONFIG_BT_MAX_CONN; j ++){			
							if(scheduled_dist[schedule[i]] > scheduled_dist[schedule[j]]){
								uint32_t temp = schedule[i];
								schedule[i] = schedule[j];
								schedule[j] = temp;
							}
						}
					}
					/*BLEX: We check the first case update is needed.
						First case update is needed when a connection blocks another connection.*/
					for(int i = 0; i <CONFIG_BT_MAX_CONN && !need_conn_update; i ++){
						...
						if(scheduled_handles[handle_now] == false) i = CONFIG_BT_MAX_CONN;
							...
							/*BLEX: We check whether a connection is blocked by even one other connections*/
							for(int j = 0; j < max_conn_int/interval && !need_conn_update; j ++){
								...
								/* BLEX: If a connection is blocked by other connection, the closest idle cell will be allocated by defering the connecions in the back*/
								if((((temp- array1[temp_i+1] < gap_1 || array1[temp_i+1]>temp) && array0[temp_i+1] >CONFIG_BT_MAX_CONN))||( array1[temp_i+1]<array1[temp_i]+ avg_conn_evt_length[handle_now]+gap_1  && array0[temp_i+1] <CONFIG_BT_MAX_CONN)  ||(array4[temp_i]>50*40/conn_int_table[handle_now]/1.25 && conn_int_table[handle_now] !=0)){
									...
									/* BLEX: If there are idle time which can be allocated to the blocked connection, we defer appropriated connection*/
									if(is_possible){ 
					...
					/*BLEX: We check the second case update is needed.
						Second case update is needed when we can squeeze the idle time more.*/
					for(int i = 0; i <CONFIG_BT_MAX_CONN && !need_conn_update ; i ++){
						int handle_now = schedule[i];
						...
						if(i != CONFIG_BT_MAX_CONN){
							if(scheduled_handles[handle_now] && handle_now != first_handle ){
								uint32_t min_gap = -1;
								int temp_i = 1;
								/*BLEX: We check whether the second update is needed considering connection intervals*/
								...
								/*BLEX: If the second update is needed, defer the connection in the back*/
			...
			/*BLEX: If BLEX operate strangely for a long time, reschedule.*/
			BLEX_cnt4 ++;
			if(BLEX_cnt4 == 1) BLEX_start = (uint32_t)k_cyc_to_ns_floor64(k_cycle_get_32());
			else if (((uint32_t)k_cyc_to_ns_floor64(k_cycle_get_32())-BLEX_start)/NSEC_PER_USEC/USEC_PER_MSEC > 5000) {

				need_conn_update = false;
				need_schedule_update = 0;
				need_update_dummy = false;
				BLEX_cnt4 = 0;
			}
		}
	...
	uint32_t gap_temp = (uint32_t)k_cyc_to_ns_floor64(k_cycle_get_32());

	gap_temp = (gap_temp-gap_start)/NSEC_PER_USEC/USEC_PER_MSEC;
}
...
#if defined(CONFIG_CSR)
	/*BLEX: We do reverse-engineering with CSR 4.0 chipset and write this code*/
uint16_t set_winoffset(struct lll_conn *lll){ 
	uint32_t now;
#if defined(CONFIG_CSR_DIF)
	if(scheduled_handle_cnt == 1){
		now = (uint32_t)k_cyc_to_ns_floor64(k_cycle_get_32())/NSEC_PER_USEC/USEC_PER_MSEC;

		uint32_t temp_time = anchor_points[0];
		int interval = lll->interval*1.25;
		if(temp_time < now) temp_time = temp_time+conn_int_table[0]*1.25*M - now;
		else {
			uint32_t temp_temp_time =-1;
			temp_temp_time = (uint32_t)(-1)/NSEC_PER_USEC/USEC_PER_MSEC;
			temp_temp_time = 4295%interval;
			temp_time = (-now+interval*M +temp_time-295);
		}
		temp_time = (temp_time+16)%interval;

		temp_time = (temp_time*4+4)/5;
		temp_time -= 1;
		return temp_time;
	}
	else if(scheduled_handle_cnt == 2){
		now = (uint32_t)k_cyc_to_ns_floor64(k_cycle_get_32())/NSEC_PER_USEC/USEC_PER_MSEC;
		...
		temp_time = (temp_time+37)%interval;

		temp_time = (temp_time*4+4)/5;
		temp_time -= 1;
		return temp_time;
	}
	else if(scheduled_handle_cnt == 3){
		now = (uint32_t)k_cyc_to_ns_floor64(k_cycle_get_32())/NSEC_PER_USEC/USEC_PER_MSEC;
		...
		temp_time = (temp_time+51)%interval;
	...
	if(scheduled_handle_cnt == 1){
		...
		temp_time = (temp_time+conn_int_table[0]/2)%interval;
		...
		temp_time = (temp_time+conn_int_table[0]*3/4)%interval;
		...
		temp_time = (temp_time+2)%interval;
...
#endif
#if defined(CONFIG_BCM)
	/*BLEX: We do reverse-engineering with broadcom chipset and write this code*/
uint16_t set_winoffset(struct lll_conn *lll){ 
	uint32_t now;
#if defined(CONFIG_BCM_DIF)
	if(scheduled_handle_cnt == 1){
		now = (uint32_t)k_cyc_to_ns_floor64(k_cycle_get_32())/NSEC_PER_USEC/USEC_PER_MSEC;
		...
		temp_time = (temp_time+27)%interval;
		...
		temp_time = (temp_time+45)%interval;
		...
		temp_time = (temp_time+56)%interval;
	...
	if(scheduled_handle_cnt == 1){
		...
		temp_time = (temp_time+conn_int_table[0]*5/8)%interval;
...
#endif
#if defined(CONFIG_BLEX)
/* BLEX: BLEX sends CONNECT_IND with winoffset which was already calculated according to the connection interval*/
uint16_t set_winoffset(struct lll_conn *lll){ 
	if(scheduled_handle_cnt == 0&& need_update_dummy == false && need_conn_update == false){
		/*BLEX: When there is no ongoing connection, we accept new connecion immediately */
		...
		uint32_t now;
		/*BLEX: We accept the new connection only when the ongoing connection's scheduling is not pending*/
		int need_cell_cnt_now = (int)(max_conn_int/(lll->interval));
		if(need_cell_cnt_now <1) need_cell_cnt_now = 1;

		/*BLEX: We check how many idle time is needed with the new connection's interval*/
		int i=0;
		for(int temp = 1; temp<need_cell_cnt_now;  temp *= 2)
			i++;
		/*BLEX: If the max idle time is not enough, the new connection is not accepted*/
		...
		/*BLEX: We calculate the time offset from now to the max idle cell considering the time array's maximum value*/
		uint32_t temp_time = array1[max_empty_cell[i]];
